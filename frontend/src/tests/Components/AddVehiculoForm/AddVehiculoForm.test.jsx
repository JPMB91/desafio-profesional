...
test("Debe mostrar el mensaje de error si 'fuelType' no se ingresa en el form", async () => {
  ...
  await waitFor(() => {
    expect(screen.getByText(/Tipo de combustible es requerido/i)).toBeInTheDocument();
  });
});
...
test("Debe mostrar todos los mensajes de error si no se ingresa ningun dato", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText(/Año de fabricación es requerido/i)).toBeInTheDocument();
      ...
      expect(screen.getByText(/Tipo de transmisión es requerido/i)).toBeInTheDocument();
      expect(screen.getByText(/Número de puertas es requerido/i)).toBeInTheDocument();
      expect(screen.getByText(/Tipo de combustible es requerido/i)).toBeInTheDocument();
